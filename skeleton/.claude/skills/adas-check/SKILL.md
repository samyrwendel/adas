---
name: adas-check
description: "Modo COMPARE/ALIGN do ADAS — sensor de saída de faixa + auto-fix. Audita o CÓDIGO contra as faixas (lanes) e reporta desvios com severidade + arquivo:linha + fix; o align corrige o mecânico. Determinístico onde dá (grep/AST), LLM só pro nuance. Use quando o pedido for auditar/alinhar aderência ao design system, i18n, ou padrões do projeto — em projeto novo ou bagunçado; gatilhos 'tem cor fora do token?', 'mistura de idioma?', 'alinha as cores', 'audita o design', 'drift'."
when_to_use: "Auditar (compare) ou auto-corrigir (align) o código contra as faixas de design/i18n/padrões. Vale pra qualquer projeto de UI; profile em .adas/profile.json."
---
...
Auto-auditoria da **governança** (universal, sem Node) fica no `scripts/check-adas.sh` (bash).
