...
placeholders=$(count_or_zero "grep -rlE --include='*.md' --include='*.css' --exclude-dir=_template '<PLACEHOLDER>|<faixa>|<NNN>|<PROJETO>|<nome>' '$SPECS_DIR' '$SKILLS_DIR' '$ADAS' '$DECISIONS' AGENTS.md 2>/dev/null | wc -l | tr -d ' '")
