#!/usr/bin/env bash
# install-hooks.sh — instala o pre-commit dos gates do ADAS (DA-166: o commit é o gate;
...
#   1. check-secrets.sh   → BLOCK (segredo staged nunca entra) — procurado em scripts/
#      e skeleton/scripts/ (o gate age sobre o STAGED do repo, não importa onde mora)
#   2. check-adas.sh      → severidade que ele já tem (frontmatter=BLOCK, resto WARN
#      visível) — só se scripts/check-adas.sh existe no repo (repo governado)
#   3. da-index.sh check  → BLOCK se o índice de DAs divergir (DA-164) — só se existem
...
# 2) auditoria do ADAS — severidade dela (frontmatter quebrado = exit 1 = bloqueia)
